...
def calculate_production_metrics(df: pd.DataFrame, production_col: str) -> Dict:
    ...
    return {
        ...
        "volatility": df[production_col].std() / df[production_col].mean()
        if df[production_col].mean() > 0
        else 0,
        "trend": "increasing"
        if df[production_col].iloc[-1] > df[production_col].iloc[0]
        else "decreasing",
    }


def plot_production_monitoring(
    df: pd.DataFrame, production_col: str, title: str, output_path: Path
):
